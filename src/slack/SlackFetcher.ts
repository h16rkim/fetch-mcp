...
export class SlackFetcher {
  ...
  private static getAccessToken(): string {
    ...
    if (!token) {
      throw new Error(
        `${Constants.ENV_SLACK_APP_USER_OAUTH_TOKEN} environment variable is not set`
      );
    }
   ...
   */
  private static parseSlackUrl(url: string): {
    channel: string;
    timestamp: string;
    threadTs?: string;
    isReply: boolean;
    ...
    if (!urlMatch) {
      throw new Error(
        "Invalid Slack message URL format. Expected format: .../archives/{CHANNEL_ID}/p{TIMESTAMP}"
      );
    }
    ...
    const formattedTimestamp =
      timestamp.substring(0, 10) + "." + timestamp.substring(10);
   ...
   */
  private static async getUserInfoObject(
    accessToken: string,
    userId: string
   ...
   */
  private static async getSlackUserInfo(
    accessToken: string,
    userId: string
  ): Promise<SlackUser | undefined> {
    try {
   ...
   */
  private static async getSpecificReply(
    accessToken: string,
    channel: string,
    threadTs: string,
    replyTs: string
   ...
   */
  private static async getMessageRepliesArray(
    accessToken: string,
    channel: string,
    timestamp: string
    ...
    try {
      const response = await fetch(
        `https://slack.com/api/conversations.replies?channel=${channel}&ts=${timestamp}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            "Content-Type": "application/json",
  ...
  private static async handleReplyMessage(
    ...
    const user = replyMessage.user
      ? await this.getUserInfoObject(accessToken, replyMessage.user)
      : undefined;
    const messageModel = new SlackMessageModel(
      replyMessage,
      channel,
      true,
      user,
  ...
  private static async handleRegularMessage(
    ...
    if (!response.ok) {
      if (response.status === 401) {
        return this.createErrorResult(
          "Authentication failed. Please check your SLACK_APP_USER_OAUTH_TOKEN"
        );
      }
      return this.createErrorResult(
    ...
    const user = message.user
      ? await this.getUserInfoObject(accessToken, message.user)
      : undefined;

    // Get replies only for original messages
    const replies = message.threadTimestamp
      ? await this.getMessageRepliesArray(
          accessToken,
          channel,
